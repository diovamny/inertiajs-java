 ...
 * Target: generated Vue starter with ssr-enabled (E2E_SSR_URL selects it;
 * defaults to the Spring starter smoke URL).
 */
...
test.describe('SSR contracts', () => {
  ...
  test('CSR fallback renders when the sidecar is down', async ({ page }) => {
    ...
    const html = await (await page.request.get(`${SSR_URL}/`)).text();
