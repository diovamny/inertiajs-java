...
export default function FlashMessages() {
  ...
  const errorsCount = Object.keys(page.props.errors).length
  ...
  return (
    <div>
      {page.props.success && show && (
        <div className="flex items-center justify-between mb-8 max-w-3xl bg-green-500 rounded">
          <div className="flex items-center">
            <svg className="shrink-0 ml-4 mr-2 w-4 h-4 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
              <polygon points="0 11 2 9 7 14 18 3 20 5 7 18" />
            </svg>
            <div className="py-4 text-white text-sm font-medium">{page.props.success}</div>
          </div>
      ...
      {(page.props.error || errorsCount > 0) && show && (
        <div className="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded">
          <div className="flex items-center">
            ...
            {page.props.error ? (
              <div className="py-4 text-white text-sm font-medium">{page.props.error}</div>
            ) : (
              <div className="py-4 text-white text-sm font-medium">
                {errorsCount === 1 ? <span>There is one form error.</span> : <span>There are {errorsCount} form errors.</span>}
              </div>
            )}
          </div>
          <button type="button" className="group mr-2 p-2" onClick={() => setShow(false)}>
