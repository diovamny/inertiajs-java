<script>
  ...
  let { organizations, filters } = $props()
      ...
      <tbody>
        {#each organizations.data as org (org.id)}
          <tr class="border-b border-gray-100 hover:bg-gray-50">
            <td class="px-6 py-4">
              <a href="/organizations/{org.id}/edit" class="text-indigo-600 hover:text-indigo-900 font-medium">
                {org.name}
              </a>
              {#if org.deleted_at}
            ...
            <td class="px-6 py-4">{org.city || ''}</td>
            <td class="px-6 py-4">{org.phone || ''}</td>
            <td class="px-6 py-4 text-right">
              <button
                onclick={() => destroy(org.id)}
