...
cd examples/quarkus/pingcrm/src/main/webui && npm i && npm run build && cd ../../..
...
cd examples/quarkus/pingcrm/src/main/webui
npm install
...
mvn -q -f examples/quarkus/pingcrm quarkus:dev     # http://localhost:8080
```
...
6. **API pública JSON** (no-Inertia): un subset de endpoints sin `X-Inertia`
   ...
   REST más".
7. **Compará con `examples/quarkus/demo-app`**: el demo-app de `quarkus-inertia` usa
...
- **README**: `examples/quarkus/pingcrm/README.md` (credenciales, comandos, decisiones).
