 ...
 * hand-written filter. Session data lives server-side, so trusting the
 * session email is safe; no database access happens here (fully
 * non-blocking).
 *
...
public class DemoSessionMechanism implements HttpAuthenticationMechanism {
    ...
    public Uni<SecurityIdentity> authenticate(RoutingContext context,
        ...
        SecurityIdentity authenticated = QuarkusSecurityIdentity.builder()
            .setPrincipal(() -> userEmail)
            .addRole("user")
            .setAnonymous(false)
            .build();
        return Uni.createFrom().item(authenticated);
    }
