...
**Protocol: 60/62 verified (5 with Playwright E2E).**
...
| 56 | Special | Explicit append() API with nested routes (append/prepend per path) | https://inertiajs.com/docs/v3/data-props/merging-props | TESTED | ✅ | ✅ | TckSpringTest, TckQuarkusTest, TckReactiveTest, MergePlanUnitTest, MergeableBuilderUnitTest | M3: MergePlan + mergeable().append().prepend().deep().matchOn() on both facades (same wire on 3 transports); legacy merge() unchanged; TCK merge-nested + reset green; E2E client proof lands in M4 |
...
| 60 | Shared | Instant-visit sharedProps behavior observed by official client | https://inertiajs.com/docs/v3/the-basics/instant-visits | IMPLEMENTADO | 🔄 | 🔄 | InertiaPageTest (both) | 0.0.5 closes this in M4 with Playwright + official Vue/React/Svelte clients (persist, update, exclude, collision) |
